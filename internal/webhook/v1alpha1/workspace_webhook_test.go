...
// ---------------------------------------------------------------------------
// Quota enforcement integration tests (via envtest + admission chain)
// The webhook suite sets MaxWorkspacesPerUser=2.
// ---------------------------------------------------------------------------

var _ = Describe("Workspace Quota Enforcement (envtest)", func() {
	const quotaUser = "quota-test-user"
	...
	It("should allow creation under quota and deny at quota", func() {
		userClient := createImpersonatedClient(quotaUser)

		By("Creating workspace 1 of 2 (should succeed)")
		ws1 := &tenantv1alpha1.Workspace{
			ObjectMeta: metav1.ObjectMeta{Name: "quota-ws-1"},
			Spec: tenantv1alpha1.WorkspaceSpec{
				Namespace: "quota-ns-1",
				Members: []tenantv1alpha1.WorkspaceMember{
					{Role: tenantv1alpha1.MemberRoleAdmin, Subject: quotaUser},
				},
			},
		}
		Expect(userClient.Create(ctx, ws1)).To(Succeed())
		defer func() { _ = k8sClient.Delete(ctx, ws1) }()

		By("Creating workspace 2 of 2 (should succeed)")
		ws2 := &tenantv1alpha1.Workspace{
			ObjectMeta: metav1.ObjectMeta{Name: "quota-ws-2"},
			Spec: tenantv1alpha1.WorkspaceSpec{
				Namespace: "quota-ns-2",
				Members: []tenantv1alpha1.WorkspaceMember{
					{Role: tenantv1alpha1.MemberRoleAdmin, Subject: quotaUser},
				},
			},
		}
		Expect(userClient.Create(ctx, ws2)).To(Succeed())
		defer func() { _ = k8sClient.Delete(ctx, ws2) }()

		By("Creating workspace 3 (should be denied — at quota)")
		ws3 := &tenantv1alpha1.Workspace{
			ObjectMeta: metav1.ObjectMeta{Name: "quota-ws-3"},
			Spec: tenantv1alpha1.WorkspaceSpec{
				Namespace: "quota-ns-3",
				Members: []tenantv1alpha1.WorkspaceMember{
					{Role: tenantv1alpha1.MemberRoleAdmin, Subject: quotaUser},
				},
			},
		}
		err := userClient.Create(ctx, ws3)
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("has reached the maximum number of workspaces"))
	})

	It("should not count workspaces where user is not admin toward quota", func() {
		userClient := createImpersonatedClient("quota-view-user")

		By("Creating a workspace where user is admin")
		ws1 := &tenantv1alpha1.Workspace{
			ObjectMeta: metav1.ObjectMeta{Name: "quota-view-ws-1"},
			Spec: tenantv1alpha1.WorkspaceSpec{
				Namespace: "quota-view-ns-1",
				Members: []tenantv1alpha1.WorkspaceMember{
					{Role: tenantv1alpha1.MemberRoleAdmin, Subject: "quota-view-user"},
				},
			},
		}
		Expect(userClient.Create(ctx, ws1)).To(Succeed())
		...
		By("Creating a workspace where user is view (should not count)")
		wsOther := &tenantv1alpha1.Workspace{
			ObjectMeta: metav1.ObjectMeta{Name: "quota-view-ws-other"},
			Spec: tenantv1alpha1.WorkspaceSpec{
				Namespace: "quota-view-ns-other",
				Members: []tenantv1alpha1.WorkspaceMember{
					{Role: tenantv1alpha1.MemberRoleAdmin, Subject: "someone-else"},
					{Role: tenantv1alpha1.MemberRoleView, Subject: "quota-view-user"},
				},
			},
		}
		seClient := createImpersonatedClient("someone-else")
		Expect(seClient.Create(ctx, wsOther)).To(Succeed())
		defer func() { _ = k8sClient.Delete(ctx, wsOther) }()

		By("Creating workspace 2 as admin (should succeed, only 1 admin workspace counted)")
		ws2 := &tenantv1alpha1.Workspace{
			ObjectMeta: metav1.ObjectMeta{Name: "quota-view-ws-2"},
			Spec: tenantv1alpha1.WorkspaceSpec{
				Namespace: "quota-view-ns-2",
				Members: []tenantv1alpha1.WorkspaceMember{
					{Role: tenantv1alpha1.MemberRoleAdmin, Subject: "quota-view-user"},
				},
			},
